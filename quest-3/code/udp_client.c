...
////////////////////////////////////////////////////////////////////////////////
float pitch; //variable for accelerometer
...
float calcRP(float x, float y, float z){
 ...
 //function that returns numebr of steps taken using pitch from accelerometer
int countStep(int steps, bool upswing, float upswingThreshold, float downswingThreshold){
  ...
  if(upswing) {
    if(pitch < upswingThreshold) { //if passes upswing threshhold value then increment step count
      steps++;
      return steps;
    }
  } else {
    if(pitch > downswingThreshold) { //if passes downswing threshhold value then increment step count
      steps++;
      return steps;
    }
  }
...
// function that returns degrees celcius value from thermistor
float thermistor() {
...
//function that returns battery voltage values
float batterymonitor(){
  ...
  float voltdivide;
  voltdivide = (((float)voltage_B/1000.00)*2000.00)/(1000.00); //voltage divider to get voltage of battery
  return voltdivide;
}
...
void app_main(){
  ...
  while (1) {
    ...
    ESP_LOGI(TAG, "Socket created");
      char payload[200]; //for sending data
      int steps = 0; //counting steps
      bool upswing = true; //for calibrating step
      float upswingThreshold = -40;
      float downswingThreshold = 15;
      int newStep;
      int water; //variable for water reminder

      float temp; //variabel for clearing buffer
      float battery;

      time_t time_2, time_3; //vraibels that hold time
      time_2 = time(NULL);
      time_3 = time(NULL);

        while (1) {
          water = 0;
          //flash blue led every 6 seconds for hydatrion reminder
          if(difftime(time(NULL), time_2) >= 6.0){
            time_2 = time(NULL);
          ...
          temp = thermistor(); //calling themistor
          battery = batterymonitor(); //calling battery monitor
          newStep = countStep(steps, upswing, upswingThreshold, downswingThreshold); //counting steps
          if (newStep != 0) {
            ESP_LOGI(TAG, " current step: %d, temp: %f, battery: %f\n", newStep, temp, battery);
            steps = newStep;
            upswing = !upswing;
            //sending data off in JSON format
            sprintf(payload, "{\"steps\":\"%d\",\"temp\":\"%f\",\"voltage\":\"%f\",\"water\":\"%d\"}\n", steps, temp, battery, water);
            ...
            if (len < 0) {
              ...
              ESP_LOGI(TAG, "%s", rx_buffer);
                // flash red LED if signal recieved
            ...
            //if step isn't changing, send data every two seconds (power saving feature)
            if(difftime(time(NULL), time_3) > 2.0){
              time_3 = time(NULL);
              ESP_LOGI(TAG, " current step: %d, temp: %f, battery: %f\n", steps, temp, battery);
              //sending data out in JSON format
              sprintf(payload, "{\"steps\":\"%d\",\"temp\":\"%f\",\"voltage\":\"%f\",\"water\":\"%d\"}\n", steps, temp, battery, water);
              int err = sendto(sock, payload, strlen(payload), 0, (struct sockaddr *)&destAddr, sizeof(destAddr));
              if (err < 0) {
                ESP_LOGE(TAG, "Error occured during sending: errno %d", errno);
              ...
              if (len < 0) {
                ...
                ESP_LOGI(TAG, "Received %d bytes from %s:", len, addr_str);
                ESP_LOGI(TAG, "%s", rx_buffer);
                //flash red LED if signal received from server
                if(strcmp(rx_buffer, "1") == 0){
                    gpio_set_level(RED_LED, 1);
                    vTaskDelay(pdMS_TO_TICKS(1000));
                    gpio_set_level(RED_LED, 0);
